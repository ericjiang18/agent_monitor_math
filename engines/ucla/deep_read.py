...
from http_utils import curl_download as _curl_download
from http_utils import urlopen as _ssl_urlopen
...
def _download(url: str, dest: Path, timeout: int = 60) -> bool:
    ...
    for attempt in range(1, n_attempts + 1):
        try:
            ...
            with _ssl_urlopen(req, timeout=timeout) as resp:
            ...
            if "CERTIFICATE_VERIFY_FAILED" in str(e) or "SSL" in str(e):
                ...
                if _curl_download(url, dest, timeout=timeout):
                    print(f"[deep_read] curl fallback succeeded for {url}")
                    return True
            if attempt < n_attempts:
                wait = _DOWNLOAD_BACKOFFS[attempt - 1]
                print(f"[deep_read] retrying in {wait}s...")
                time.sleep(wait)
    print(f"[deep_read] urllib failed for {url}; trying curl fallback")
    if _curl_download(url, dest, timeout=timeout):
        print(f"[deep_read] curl fallback succeeded for {url}")
        return True
...
def _parse_extraction(text: str, arxiv_id: str, n_lemmas: int) -> dict:
    ...
    for lem in raw[:n_lemmas]:
        ...
        if not stmt:
            continue
        cleaned.append({
            "label":         (lem.get("label") or "").strip()[:80],
            "statement":     stmt,
            "proof_sketch":  (lem.get("proof_sketch") or "").strip()[:4000],
        })
    obj["lemmas"] = cleaned
    return obj
