...
Pipeline:
  ...
  Stage 1  │ Literature research (LIT_ENABLED=1, default on): one
  ...
  Stage 2  │ Advisor directions: read the Stage 0 literature extractions
  ...
  Stage 3 │ Deep-Read (optional, DEEP_READ_ENABLED=1): triage ≤N
  ...
  Stage 4  │ Single-Advisor Orchestrated Solving                      [advisor: web_search on]
  ...
  Stage 5 │ Assembly: after the advisor loop ends, an advisor reviews
             the shared KB and drafts an assembly prompt, then a solver
             assembles the best possible self-contained solution.
  Stage 6  │ Verify + Refine (VERIFY_ROUNDS rounds) on the assembled
  ...
  Stage 7 │ Finalize (optional, FINALIZE_ENABLED=1): two-track
             final-output stage.
             ...
             → solution.tex
  Stage 8  │ Verified solution → benchmark.json                      [BENCHMARK_MODEL]
  Stage 9  │ Verified solution → solution.tex  (LaTeX typesetting)   [TYPESET_MODEL]
...
load_local_env_file(SCRIPT_DIR.parent / ".env")
...
BENCHMARK_MODEL  = os.getenv("BENCHMARK_MODEL",  "gpt-5-mini")
SUMMARIZE_MODEL  = os.getenv("SUMMARIZE_MODEL",  "gpt-5-mini")
TYPESET_MODEL    = os.getenv("TYPESET_MODEL",    "gpt-5-mini")
...
MAJOR_GAP_BUDGET       = env_int("MAJOR_GAP_BUDGET",       3)  # extra advisor calls for major gap review, not counted against ADVISOR_BUDGET
GIVE_TIME_TO_FIX       = env_bool("GIVE_TIME_TO_FIX",      True)   # grant extra budget rounds on confirmed major gaps
GIVE_TIME_TO_FIX_MAX   = env_int("GIVE_TIME_TO_FIX_MAX",   3)      # max extra rounds granted across the whole run
...
SKIP_BENCHMARK                = env_bool("SKIP_BENCHMARK",                  False)
...
HARNESS_OUTPUTS_LOG = Path(os.getenv(
    "LLM_HARNESS_OUTPUTS",
    str(Path(__file__).resolve().parent.parent / "monitor" / "harness_outputs.jsonl"),
))
LOG_FILE.touch()
...
_TRACE_ROOT = os.getenv("LLM_TRACE_ID", "").strip() or OUTPUT_ROOT_DIR.name
...
_harness_outputs_lock  = threading.Lock()
_solver_history_lock   = threading.Lock()

# ─── Major gap queue ──────────────────────────────────────────────────────────
# Thread-safe queue for major gaps detected during background verification.
_patches_by_task: dict = {}  # stage_label → patches_text from verifier
_gaps_by_task: dict = {}     # stage_label → (major_gaps_text, minor_gaps_text)
_give_time_used: int = 0     # number of GIVE_TIME_TO_FIX extra rounds granted so far
# Items: (task_id, major_gaps_text, solution_text, problem_solved)

_major_gap_queue: queue.Queue = queue.Queue()  # (task_id, major_gaps_text, solution_text, problem_solved)
...
def _extract_openai_response_detail(response) -> tuple[str, str | None, list]:
    """Pull output text, reasoning summary, and tool calls from a Responses API object."""
    ...
    for item in getattr(response, "output", None) or []:
        ...
        if kind == "reasoning":
            ...
            if isinstance(summary, list):
                thinking_parts.extend(str(s) for s in summary if s)
            elif summary:
            ...
            if not output_text:
                for block in getattr(item, "content", None) or []:
                    ...
                    if bt in ("output_text", "text") and text:
                        output_text = output_text + text if output_text else text
    ...
    return output_text, thinking, tool_calls


def _log_harness_output(stage_name: str, prompt: str, response, usage: dict) -> None:
    """Sidecar log with full prompt/output for dashboard (proxy often omits Responses API text)."""
    output_text, thinking, tool_calls = _extract_openai_response_detail(response)
    row = {
        "kind":         "harness_output",
        "ts":           datetime.now(timezone.utc).isoformat(),
        "trace_id":     f"{_TRACE_ROOT}::{stage_name}",
        "trace_name":   os.getenv("LLM_TRACE_NAME", _TRACE_ROOT),
    ...
    HARNESS_OUTPUTS_LOG.parent.mkdir(parents=True, exist_ok=True)
    with _harness_outputs_lock:
        with open(HARNESS_OUTPUTS_LOG, "a", encoding="utf-8") as f:
    ...
    while True:
        ...
        try:
            ...
            trace_headers = {
                ...
                "x-trace-name": os.getenv("LLM_TRACE_NAME", _TRACE_ROOT),
                "x-caller":     stage_name,
            }
            if harness_meta:
                trace_headers["x-harness-meta"] = json.dumps(harness_meta, ensure_ascii=False)
    ...
    def assembly_task_index_for_advisor(self):
        ...
        with self._state_lock:
            ...
            for _, tid, rnd, kind, vtag, is_relax, ps, desc, ps_label, rel_label in rows:
                ...
                lines.append(head)
                if ps:
                    snippet = ps if len(ps) <= 240 else ps[:240].rstrip() + "…"
                    lines.append(f"      {ps_label}: {snippet}")
            return "\n".join(lines)
...
def run_solver_task(advisor_prompt, task_id, round_num):
    ...
    solver_text, usage = run_response(
        ...
        harness_meta={
            ...
            "round_id": round_num,
            "agent_id": task_id,
            "received_from": [f"advisor_r{round_num}"],
        },
    )
...
def spawn_writeup_from_advisor(writeup_task, round_num):
    ...
    try:
        ...
        description_text = (
            f"writeup (full problem): {stmt[:80]}"
            if not is_relaxation_final
            else f"writeup (partial): {stmt[:80]}"
        )
...
def run_advisor_round(round_num, remaining_budget, last_solver_outputs,
    ...
    advisor_text, usage = run_response(
        ...
        web_search=True,
        harness_meta={
            "role": "orchestrator_advisor",
            "round_id": round_num,
            "agent_id": f"advisor_r{round_num}",
            "received_from": [f"advisor_r{round_num - 1}"] if round_num > 1 else ["advisor_directions", "kb"],
            "sent_to": [],  # filled post-plan in conversation.jsonl enrichment
        },
    )
    print(f"\n[advisor r{round_num}]:\n{advisor_text}")
...
def orchestrated_solve_loop_v2(directions):
    ...
    if not skip_loop:
        while budget > 0:
            ...
            # ── Drain major gap queue from background verification ────────
            major_gap_reviews = []
            while not _major_gap_queue.empty():
                try:
                    major_gap_reviews.append(_major_gap_queue.get_nowait())
                except queue.Empty:
                    break

            if major_gap_reviews:
                # Call advisor for major gap review WITHOUT consuming budget
                major_gap_budget_used = 0
                for (mgap_task_id, mgap_text, mgap_solution, mgap_problem_solved) in major_gap_reviews:
                    if major_gap_budget_used >= MAJOR_GAP_BUDGET:
                        # Budget for major gap reviews exhausted — treat as minor and move on
                        print(f"[major_gap] Budget exhausted, treating gap in {mgap_task_id} as minor")
                        continue

                    # Call advisor to confirm whether the gap is truly major
                    confirmed, advisor_guidance = _run_major_gap_advisor_review(
                        task_id=mgap_task_id,
                        major_gaps=mgap_text,
                        solution=mgap_solution,
                        problem_solved=mgap_problem_solved,
                        round_num=round_num,
                    )
                    major_gap_budget_used += 1

                    if confirmed:
                        # Write to KB as major gap
                        memory.add_failed_attempt(
                            approach=f"Task {mgap_task_id}: {mgap_problem_solved[:100]}",
                            reason=f"Major gap confirmed by advisor: {mgap_text[:300]}",
                            source_plan=f"major_gap_review_r{round_num}_{mgap_task_id}",
                            turn=round_num,
                        )
                        
                        # # Decrement task_counter so solver retries the same turn number
                        # task_counter -= 1
                        # task_counter = max(1, task_counter)

                        # Grant an extra budget round so the solver is not penalized
                        # for the major gap (up to GIVE_TIME_TO_FIX_MAX total)
                        global _give_time_used
                        if GIVE_TIME_TO_FIX and _give_time_used < GIVE_TIME_TO_FIX_MAX:
                            budget += 1
                            _give_time_used += 1
                            print(f"[give_time_to_fix] Granted 1 extra round for {mgap_task_id} "
                                  f"({_give_time_used}/{GIVE_TIME_TO_FIX_MAX} used). "
                                  f"Budget now {budget}.")
                        # Store advisor guidance to inject into next solver prompt
                        last_solver_outputs.append({
                            "task_id": mgap_task_id,
                            "description": f"[MAJOR GAP — retry] {mgap_text[:100]}",
                            "output": f"MAJOR GAP FOUND:\n{mgap_text}\n\nADVISOR GUIDANCE:\n{advisor_guidance}",
                            "usage": {},
                        })
                    else:
                        # Advisor downgraded to minor — run refine in next round
                        print(f"[major_gap] Advisor downgraded gap in {mgap_task_id} to minor")
...
def Run_Verify(solution, stage_label, *, original_problem, claim):
    ...
    # Store gaps+patches for potential use in finalize polish step
    _gaps_by_task[stage_label] = (major_gaps_text, minor_gaps_text_clean)
    if patches_text and patches_text.strip():
        _patches_by_task[stage_label] = patches_text
...
def _run_major_gap_advisor_review(task_id, major_gaps, solution, problem_solved, round_num):
    """Ask advisor to confirm whether a gap is truly major. Returns (confirmed: bool, guidance: str)."""
    prompt = f"""A verifier flagged the following as MAJOR GAPS in task {task_id}:

{major_gaps}

The solution was attempting to prove:
{problem_solved[:500]}

Please review these gaps. Answer:
1. Do you CONFIRM these are major gaps that require a new approach? (yes/no)
2. If yes, provide brief guidance for the solver's retry (what to do differently).
3. If no, explain why they are actually minor/fixable.

Format:
<CONFIRMED>yes or no</CONFIRMED>
<GUIDANCE>your guidance here</GUIDANCE>"""

    output_text, usage = run_response(
        prompt,
        stage_name=f"major_gap_review_r{round_num}_{task_id}",
        reasoning_effort="high",
        verbosity="medium",
        max_output_tokens=4096,
        web_search=False,
    )
    confirmed_m = re.search(r"<CONFIRMED>(.*?)</CONFIRMED>", output_text, re.DOTALL | re.IGNORECASE)
    guidance_m  = re.search(r"<GUIDANCE>(.*?)</GUIDANCE>",   output_text, re.DOTALL | re.IGNORECASE)
    confirmed   = confirmed_m and confirmed_m.group(1).strip().lower() == "yes" if confirmed_m else False
    guidance    = guidance_m.group(1).strip() if guidance_m else ""
    print(f"[major_gap_review] task={task_id} confirmed={confirmed}", flush=True)
    return confirmed, guidance
...
def verify_refine_stage(i, plan):
    ...
    for o in range(VERIFY_ROUNDS):
        ...
        if verdict_class == "incorrect":
            # Major gap → enqueue for Stage 2 advisor review, return immediately
            # with if_final_true="false" (merged with the legacy "major_gap" label
            # — nothing downstream distinguishes them).
            _major_gap_queue.put((i, major_gaps_text, now_solution, problem_solved))
            print(f"[verify_{i}] verdict=Incorrect → queued for advisor review, skipping refine")
        ...
        # verdict_class == "correct_after_minor": run refine to close the minor
        # gaps using the verifier's patches (already concatenated into
        # minor_gaps_text by Run_Verify).
        refine_prompt = REFINE_PROMPT_TEMPLATE.format(
            original_problem=problem, claim=problem_solved,
            solution=now_solution, gaps=minor_gaps_text,
        )
    ...
    if if_final_true == "None":
        ...
        if verdict_class == "correct":
            ...
            _major_gap_queue.put((i, major_gaps_text, Final_Solution, problem_solved))
            print(f"[verify_{i}] Final pass: verdict=Incorrect — queued for advisor review")
            return memory.add_final_solution(
                task_id        = i,
                if_final_true  = "false",
                final_solution = Final_Solution,
...
# ── Stage 8: Benchmark documentation ──────────────────────────────────────────
